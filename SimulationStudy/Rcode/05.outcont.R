...
################################################################################
# SIMULATED SCENARIO: CONTINUOUS OUTCOME
################################################################################
...
data$effexp <- data$cumexp*beta

# SIMULATE THE OUTCOME (RANDOM SUBJECT-SPECIFIC BASELINE + EFF + ERROR)
data[, y := runif(1,50,150)+effexp+rnorm(length(x),0,20), by=id]
plot(y~date, data[id%in%1:5], ylab="Outcome", main="Outcome distribution",
  col=alpha(data[id%in%1:5]$id, 0.3))
abline(h=data[id%in%1:5,.(y=mean(y)),by=id]$y, col=1:5, lwd=2)
mtext("First 5 subjects")

# RUN THE MODEL
mod <- gnm(y ~ cumexp, eliminate=id, data=data, family="gaussian")
