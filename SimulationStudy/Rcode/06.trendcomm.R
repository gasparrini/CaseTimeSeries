...
################################################################################
# SIMULATED SCENARIO: COMMON TREND
################################################################################
...
plot(date, frrtrend(date,tpar), type="l", ylim=c(0.5,1.5), col=2, ylab="RR",
  main="Seasonal trend")
abline(h=1)
...
# COMPUTE RISK ASSOCIATED TO EXPOSURE AND TREND
data$rrexp <- exp(data$cumexp*beta)
data$rrtrend <- frrtrend(data$date, tpar)

# SIMULATE THE OUTCOME (5-20 EVENTS)
data[, y := rmultinom(1, sample(5:20, 1), prob=rrexp*rrtrend), by=id]

# DEFINE CYCLIC SPLINES FOR SEASONALITY
splseas <- pbs(yday(data$date), df=6)

# RUN THE MODEL
mod <- gnm(y ~ cumexp + date + splseas, eliminate=id, data=data,
  family="poisson")
