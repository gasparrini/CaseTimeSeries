...
################################################################################
# SIMULATED SCENARIO: UNOBSERVED BASELINE CONFOUNDER
################################################################################
...
data[, y := rmultinom(1, round(first(zfix)/5+1), prob=rrexp*rrtrend), by=id]
...
mod <- gnm(y ~ cumexp + date + splseas, eliminate=stratum, data=data,
  family="poisson")
