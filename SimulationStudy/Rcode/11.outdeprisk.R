...
################################################################################
# SIMULATED SCENARIO: OUTCOME-DEPENDENT RISK
################################################################################
...
# SIMULATE CHANGE OF STATUS THAT PREVENTS FUTURE OUTCOMES
data[, change := cumsum(rbinom(length(y), 1, 0.2) * y>0) > 0, by=id]
data[, y := ifelse(seq(length(y)) < min(c(length(y), which(change))) + 1, y, 0),
  by=id]

# PLOT
plot(date, date, ylim=c(0.5,10+0.5), yaxt="n", ylab="", xlab="Follow-up",
  frame.plot=F, main="Outcome-dependent risk status")
